...
getUniqueGameIDs(DATA).forEach(x =>{
  gamesArray.push(gameBuilder(DATA, x))
})
...
const Schedule = () => (
  <div>
    <ul>
      ...
      {
        [...Array(gamesArray.length).keys()].map(n => {
            return <li key={n}>
                    {/* <Link to={`/schedule/${n+1}`}>
                      Schedule {n+1}
                    </Link> */}
                    <Link to={`/schedule/${n}`}>
                      {gamesArray[n].gameDate + " | " + gamesArray[n].gameTime + " | " + gamesArray[n].team1Name + " vs. " + gamesArray[n].team2Name}
                    </Link>
                  </li>;
        })
      }
